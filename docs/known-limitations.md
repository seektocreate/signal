...
## Theme retry-after-failure: status doesn't advance

When a previous Theme run failed (e.g., hallucinated tweet_id) and a subsequent run succeeds, the digest status remains at `'filtering'` instead of advancing to `'writing'`. Manual bump required to unblock Writer. Diagnostic: check `agent_runs` for paired theme rows where the latest is clean but the digest status hasn't advanced.
